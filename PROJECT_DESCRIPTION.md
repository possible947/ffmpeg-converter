...
`ffmpeg_converter` — кроссплатформенный CLI‑инструмент для формирования и выполнения команд `ffmpeg` с набором преднастроек: поддержка ProRes (`prores`, `prores_ks`) и `copy`, нормализация аудио (peak, loudness), двухпроходный анализ (peak 2‑pass, loudnorm 2‑pass), прогресс‑бар и интерактивное текстовое меню.
...
- `-c, --codec <copy|prores|prores_ks>` — видео кодек. `copy` означает копирование видеопотока; `prores` и `prores_ks` используют соответствующие кодеки.
