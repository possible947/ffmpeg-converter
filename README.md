...
`ffmpeg_converter` — кроссплатформенный инструмент командной строки для формирования и запуска оптимизированных команд `ffmpeg` с поддержкой ProRes, копирования видеопотока и нескольких режимов нормализации аудио (включая двухпроходную EBU R128).

Особенности:

- Формирование команд `ffmpeg` для различных сценариев (copy, `prores`, `prores_ks`).
