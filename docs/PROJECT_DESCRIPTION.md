...
- CMake now generates `build/generated/presets_v2.json` with build-time
   availability for the target platform and HQ_converter capability, and
   packages it beside every supported CLI/GUI output.
- Runtime hardware probes now gate component detection through the generated
   `presets_v2.json` catalog only; the legacy execution catalog is not used for
...
- The structured selection catalog is consumed by the C CLI; GUI consumption
   remains the next implementation stage. First-level availability is generated
